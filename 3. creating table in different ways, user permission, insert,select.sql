...
create table sales.departments(id serial primary key, dept_name VARCHAR, emp_id serial, created_at date);
--here employees id mentioned as emp_id, but kept data type same , so we can use different columns names in primary and foreign key but data type should be same
